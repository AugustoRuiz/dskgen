...
int main(int argc, const char** argv)
{
	...
	if (!opt.BootFile.empty()) {
		int bootResult = disk.AddBootFile(opt.BootFile);
		if (bootResult != 0) {
			cout << "ERROR: Couldn't load boot file '" << opt.BootFile << "'." << endl;
			switch (bootResult) {
			case -1:
				cout << "       File '" << opt.BootFile << "' is too big for reserved tracks (" << opt.DiskParams.reservedTracks << ")." << endl;
				break;
			case -2:
				cout << "       File '" << opt.BootFile << "' was not found." << endl;
				break;
			}
		}
	}
